...
#ifdef QEngine_Text
static inline int qLenStr(char* text) { int i=0; while(text[i] != '\0') i++; return i; }
static inline void qCopy(char* dest, char* src) { int i=0; while(src[i]) { dest[i] = src[i]; i++; } dest[i] = '\0'; }
...
static inline void qReverse(char* text) { int l = qLength(text), a = 0, b = l - 1; while (a < b) { char t = text[a]; text[a] = text[b]; text[b] = t; a++; b--; } }
#endif
