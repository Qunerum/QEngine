...
{
    static readonly string Version = "0.3.2";
    ...
    public override void Init()
    {
        ...
        progress.hideHandle = true;
        progress.isInteractable = false;
        progress.fillColor = new(50, 200, 50);
        progress.SetValue(0);
        progress.SetMax(8);

        string zip = "TemplateProject";
        ...
        progress.SetValue(2);
        Download($"https://github.com/Qunerum/QEngine/releases/download/{Version}/QEngine.dll",
            Path.Combine(engineDir, "QEngine.dll")).Wait();

        Console.WriteLine($"Downloading '{zip}.zip'...");
        progress.SetValue(3);
        Download($"https://github.com/Qunerum/QEngine/releases/download/{Version}/{zip}.zip",
            Path.Combine(engineDir, $"{zip}.zip")).Wait();
        ...
        CopyDirectory(Path.Combine(libsDir, zip), Path.Combine(engineDir, zip));
        Directory.Delete(Path.Combine(libsDir, zip), true);
...
dotnet ""%~dp0QEngine.dll"" %*");
    ...
    public static void CopyDirectory(string sourceDir, string targetDir)
    {
        ...
        {
            Console.WriteLine($"Copying '{Path.GetFileName(dir)}'...");
